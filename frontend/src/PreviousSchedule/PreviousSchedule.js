      ...
      componentDidMount(){
        ...
        this.props.scheduleDate + "&end=" + end).then((response) => {
          ...
          for(let i =0; i < returnedData.length; i++){
            ...
            videos.push( <SingleSchedule title="From Broadcast" startTime = {moment(returnedData[i].published_time.start).format("HH:mm:ss")}
            duration={returnedData[i].published_time.duration}  prev={prev}/>)
