...
class Schedule extends React.Component {
  ...
  makeScheduleEvent(broadcast) {
    ...
    start.set({
      hour: broadcast.startTime.charAt(0) + broadcast.startTime.charAt(1),
    ...
    let startXML = `<TVAMain xmlns="urn:tva:metadata:2007" xmlns:mpeg7="urn:tva:mpeg7:2005" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" 
  ...
  savePlaylist() {
    if (videos.length > 0) {
      ...
      pids.forEach(async i1 =>
        loadedContent.forEach(async (i2, idx) => {
          if (
            i1 === i2.available_versions.version[0].pid ||
            i1 === i2.versionPid
          ) {
            await axios
              .get(`/version_crid?pid=${i1}`)
              .then(response => {
